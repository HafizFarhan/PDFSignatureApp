...
function App() {
    ...
    debounce((e, data) => {
      ...
      requestAnimationFrame(() => {
        if (pdfDoc) {
          ...
          const numOfPages = pageHeights.length;
          ...
          const targetPageIndex = pageHeights.findIndex((pageHeight, index) => {
            const accumulatedHeight = pageHeights
              ...
              .reduce(
                (sum, height) =>
                  sum +
                  height * calculateValue(0.2, index, numOfPages, 0.002) * 10,
                0
              );
            return (
              y <
              accumulatedHeight +
                pageHeight * calculateValue(0.2, index, numOfPages, 0.002) * 10
            );
          });

          if (targetPageIndex == -1) {
            setCurrentPageNumber(targetPageIndex + 2);
          } else {
  ...
  function calculateValue(
    ...
    const endValue = startValue + proportionIncrement;
    const step = (endValue - startValue) / (lastIndex - 1);
    return startValue + step * (currentIndex - 1);
  }
    ...
    try {
      ...
      if (signature) {
        ...
        targetPage.drawImage(pngImage, {
          x: finalX,
          y: finalY + signaturePosition.y * 0.72,
      ...
      for (let i = 0; i < textAnnotations.length; i++) {
        ...
        var pageNo = Math.floor((y * 0.99) / pageHeights[0]);
        console.log("y ", y * 0.99);
        console.log("Pages height ", pageHeights[0] * pageHeights.length);
        ...
        const adjustedY =
          height *
          ((pdfPageHeight - (y - pageNo * pdfPageHeight)) / pdfPageHeight);
  ...
  return (
    <div className="App">
      ...
      <div className="navbar">
        <div>
          <img className="logo" src={logo} alt="INBOXACCEL"></img>
        </div>
